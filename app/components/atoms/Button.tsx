...
const Button: React.FC<Props> = ({ children, className, variants = 'primary', disabled, size = 'base', ...props }) => {
    const buttonClass = getButtonClasses(variants, size, !!disabled, className);

    return (
        <button disabled={disabled} className={`${buttonClass} ${className} hover:`} {...props}>
            {children}
        </button>
    );
};
